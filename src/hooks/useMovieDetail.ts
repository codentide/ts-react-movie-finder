import type { MovieDetail, MovieDetailFromApi, MovieId } from '../types'
import { useEffect, useState } from 'react'
import { formatMovieDetail } from '../utils'
...
export function useMovieDetail(movieId: MovieId | undefined): {
  ...
  async function fetchMovie(url: string): Promise<MovieDetailFromApi | null> {
    setIsLoading(true)
    setError(null)
    ...
    try {
      ...
      if (data) {
        return data as MovieDetailFromApi
      } else {
  ...
  useEffect(() => {
    ...
    async function getMovies() {
      ...
      const movieDetail = await fetchMovie(`${BASE_URL}${endpoint}`)

      if (movieDetail) {
        const formattedMovie = formatMovieDetail(movieDetail)
